#!/usr/bin/env python3
"""
Simplified Fixture Validator
Validates all protocol fixtures against the Spinner API with real gas prices.
Uses JSON decoder that handles concatenated JSON objects properly.
"""
...
# Supported chains (have gas data in production Spinner)
SUPPORTED_CHAINS = {
...
def load_ndjson(file_path: Path) -> List[Dict]:
    """Load NDJSON file (multiple JSON objects separated by newlines)"""
    intents = []
    with open(file_path, 'r') as f:
        content = f.read()
        # Use JSONDecoder to handle multiple root objects
...
def validate_intent(intent: Dict, gas_cache: Dict[int, Optional[Dict]]) -> ValidationResult:
    ...
    token_prices = {
        ...
        137: 0.9,   # POL
        56: 600,    # BNB
        59144: 3000, # ETH (Linea)
    }

    if src_gas_gwei is not None:
        token_price = token_prices.get(src_chain, 1)
