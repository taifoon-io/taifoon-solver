...
//!
//! All three fields are optional, but at least one must be present (a
...
impl RulePredicate {
    fn is_useful(&self) -> bool {
        self.min_amount_usd.is_some() || self.max_gas_gwei.is_some() || self.dst_chain.is_some()
    }
}
    ...
    pub fn evaluate(
        ...
        for r in &self.rules {
            ...
            let reason = r.description.clone().unwrap_or_else(|| {
                format!(
                    "skip_rule:min_amount_usd={:?},max_gas_gwei={:?},dst_chain={:?}",
                    r.predicate.min_amount_usd, r.predicate.max_gas_gwei, r.predicate.dst_chain
                )
            });
            return Some(reason);
        }
...
mod tests {
    ...
    fn fires_when_all_present_predicates_match() {
        ...
        assert!(r.evaluate("across", 1, Some(150.0), Some(60.0)).is_some());
        // amount above 200 → keep
        assert!(r.evaluate("across", 1, Some(250.0), Some(60.0)).is_none());
        // gas below 50 → keep
        assert!(r.evaluate("across", 1, Some(150.0), Some(40.0)).is_none());
    }
    ...
    fn protocol_isolation() {
        ...
        assert!(r.evaluate("debridge", 1, Some(50.0), None).is_none());
        assert!(r.evaluate("across", 1, Some(50.0), None).is_some());
    }
    ...
    fn dst_chain_filter() {
        ...
        assert!(r.evaluate("across", 1,     Some(50.0), None).is_none());
        assert!(r.evaluate("across", 42161, Some(50.0), None).is_some());
    }
}
