#!/usr/bin/env bash
# Mainnet verification gate — Phase 2 (across first-fill).
set -euo pipefail
REPO_ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
LEDGER="$REPO_ROOT/outcomes/verification_gates.jsonl"
mkdir -p "$REPO_ROOT/outcomes"

DRY_RUN=false MAX_NOTIONAL_USD="${MAX_NOTIONAL_USD:-50}" PROTOCOL_FILTER=across \
...
while (( $(date +%s) < DEADLINE )); do
    ...
    if [[ -n "$DB" ]]; then
        ROW=$(sqlite3 "$DB" "select tx_hash, actual_profit_usd, intent_id, dst_chain from solver_outcomes where decision='confirmed' and protocol like '%across%' and tx_hash is not null order by ts desc limit 1" || true)
        if [[ -n "$ROW" ]]; then
            ...
            echo "==> Phase 2 mainnet gate PASS: tx=$tx profit=$profit chain=$chain"
            printf '{"phase":2,"tx":"%s","explorer":"%s","ts":"%s","intent_id":"%s","dst_chain":%s,"realized_usd":%s}\n' \
...
echo "==> Phase 2 mainnet gate TIMED OUT after 30 minutes"
exit 1
